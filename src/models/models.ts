...
export class Models {
  ...
  constructor(public conn: mongoose.Connection) {
    ...
    let accesses = [];
    this.permissions.accesses = accesses;
    fs.readdirSync(`${__dirname}`).forEach(f => {
      if (f.endsWith(".yaml")) {
        try {
          let id = f.slice(0, -5)
          let schemaConfig = yaml.load(`${__dirname}/${f}`)
          console.log("MODEL VIEWS", schemaConfig);
          if (schemaConfig.$view && typeof schemaConfig.$view.permissions !== "undefined") {
            if (schemaConfig.$view.permissions) {
              accesses.push({ id: id, permissions: schemaConfig.$view.permissions })
            }
          } else {
            accesses.push({ id: id, permissions: { create: true, read: true, update: true, delete: true } })
          }
  ...
  private async initDB() {
    ...
    ;[
      ...
      let role = await this._models.role.findOne({ code: rn.code })
      if (!role) {
        this._models.role.create(rn)
      }
    })
